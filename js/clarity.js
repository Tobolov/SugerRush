...
Clarity.prototype.load_map = function (id) {
	var map = this.maps[id === -1 ? this.mapId : id];
	...
	this.player.loc.x = (this.playernumber === 1 ? map.player1.x : map.player2.x) * this.tile_size || 0;
	this.player.loc.y = (this.playernumber === 1 ? map.player1.y : map.player2.y) * this.tile_size || 0;
	this.player.colour = (this.playernumber === 1 ? map.player1.colour : map.player2.colour) || '#000';
	...
	if(id !== -1) {
		this.current_map = JSON.parse(JSON.stringify(this.maps[id]));
		this.mapId = id;
	...
	{
		this.log('Successfully respawned.');
	}
...
Clarity.prototype.draw_friend = function (context) {
	context.fillStyle = this.friend.colour;
	context.beginPath();
	context.arc(
		this.friend.loc.x + this.tile_size / 2 - this.camera.x,
		this.friend.loc.y + this.tile_size / 2 - this.camera.y,
		this.tile_size / 2 - 1,
		0,
		Math.PI * 2
	);
...
Clarity.prototype.draw_deaths = function (context) {
	context.font = "20px Arial";
	context.fillStyle = "#bdc3c7";
	context.fillText("Fails: " + this.deaths, 10,25);
};
...
Clarity.prototype.draw = function (context) {
	...
	this.draw_deaths(context);
	if(this.friend !== null) {
		this.draw_friend(context);
	}
};
